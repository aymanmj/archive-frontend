...
export default function FilePreview({
  ...
  return (
    <div className="fixed inset-0 z-50" aria-modal="true" role="dialog">
      ...
      <div className="absolute inset-0 p-3 md:p-6 flex" dir="rtl">
        <div className="relative bg-white rounded-2xl shadow-xl border w-full max-w-6xl mx-auto my-auto flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2 border-b">
            ...
            <div className="flex items-center gap-2">
              <a href={url} target="_blank" rel="noreferrer" className="rounded-xl border px-3 py-1.5 text-sm hover:bg-gray-50">تنزيل</a>
              <button onClick={onClose} className="rounded-xl border px-3 py-1.5 text-sm hover:bg-gray-50">إغلاق</button>
            </div>
          </div>
          ...
          <div className="flex-1 bg-gray-50">
            ...
            {isPdf(ext) && (
              <div className="w-full h-[80vh]">
                <iframe title={file.fileNameOriginal} src={`${url}#toolbar=1&navpanes=0`} className="w-full h-full" />
              </div>
            )}
            {isVideo(ext) && (
              <div className="w-full h-full overflow-auto flex items-center justify-center p-4">
            ...
            {!isImage(ext) && !isPdf(ext) && !isVideo(ext) && !isAudio(ext) && (
              <div className="p-6 text-center text-gray-600">
                ...
                <div className="mt-2">
                  <a href={url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">اضغط للتنزيل</a>
                </div>
                <div className="text-xs text-gray-400 mt-3">(يدعم المعاينة: الصور، PDF، الفيديو، الصوت)</div>
              </div>
            )}
          </div>
        </div>
