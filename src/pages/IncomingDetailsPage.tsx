...
type BasicUserOption = { id: number; fullName: string; departmentId: number | null };
...
export default function IncomingDetailsPage() {
  ...
  const isPrivileged = useMemo(() => {
    ...
    return me.roles.includes("SystemAdmin") || me.roles.includes("DepartmentManager");
  }, [me]);

  function formatDateTime(v: string | null | undefined) {
    if (!v) return "—";
    const d = new Date(v);
    return d.toLocaleString("ar-LY", { hour12: true, dateStyle: "short", timeStyle: "medium" });
  }
    ...
    async function loadAll() {
      ...
      try {
        ...
        const [recRes, meRes, depRes, usersRes] = await Promise.all([
          apiClient.get(`/incoming/${id}`),
          apiClient.get(`/users/me`),
          apiClient.get(`/departments`),
          apiClient.get(`/users/list-basic`),
        ]);

        const recData: IncomingDetails = recRes.data;
        const meData: CurrentUserInfo = meRes.data;
        const depData: any[] = depRes.data;
        const usrData: any[] = usersRes.data;
  ...
  async function handleAddFollowup(e: React.FormEvent) {
    ...
    try {
      ...
      if (isPrivileged) {
        if (targetDepartmentId) bodyToSend.targetDepartmentId = Number(targetDepartmentId);
        if (assignedToUserId) bodyToSend.assignedToUserId = Number(assignedToUserId);
      }

      await apiClient.post(`/incoming/${id}/followup`, bodyToSend);
  ...
  if (loading) {
    return (
      <div className="bg-slate-100 min-h-screen p-4 flex flex-col items-center text-center text-gray-600" dir="rtl">
        ...جاري التحميل
      </div>
    );
  }
  ...
  if (error || !details) {
    return (
      <div className="bg-slate-100 min-h-screen p-4 flex flex-col items-center text-center text-red-600" dir="rtl">
  ...
  return (
    <div className="bg-slate-100 min-h-screen p-4 flex flex-col items-center" dir="rtl">
      <div className="max-w-4xl mx-auto space-y-6 w-full">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold text-slate-800">تفاصيل المعاملة</h1>
        ...
        <div className="w-full bg-white rounded-lg shadow border border-slate-300 p-4 text-right text-slate-800">
          ...
          <div className="flex flex-col md:flex-row md:justify-between text-sm text-slate-700 leading-relaxed">
            <div className="md:w-1/2 md:pr-4 md:border-l md:border-slate-300 md:ml-4">
              <div className="mb-2">
                ...
                <span className="font-semibold">{details.externalParty?.name || "—"}</span>
              </div>
              ...
              <div className="mb-2">
                ...
                <span className="font-semibold">{details.requiredAction || "—"}</span>
              </div>
            </div>
            ...
            <div className="md:w-1/2 md:pl-4">
              ...
              <div className="mb-2">
                ...
                <span className="font-semibold">{formatDateTime(details.receivedAt)}</span>
              </div>

              <div className="mb-2">
                <span className="text-slate-500">مستلم المعاملة:</span>{" "}
                <span className="font-semibold">{details.receivedByUser || "—"}</span>
              </div>
            </div>
          </div>
          ...
          <div className="text-right">
            ...
            <div className="text-base font-medium">{details.document?.title || details.document?.summary || "—"}</div>
          </div>
        </div>
        ...
        <div className="w-full bg-white rounded-lg shadow border border-slate-300 text-slate-800">
          <div className="border-b border-slate-300 px-4 py-2 bg-slate-50 rounded-t text-center">
            <h3 className="text-lg font-semibold">المرفقات (النسخة الموقعة / PDF / صورة المسح الضوئي)</h3>
          </div>
                ...
                {details.document?.files?.length ? (
                  details.document.files.map((f) => (
                    <tr key={f.id} className="border-b border-slate-200 last:border-b-0">
                      ...
                      <td className="p-2 font-mono text-sm break-all">{f.fileNameOriginal}</td>
        ...
        <div className="w-full bg-white rounded-lg shadow border border-slate-300 text-slate-800">
          ...
          <div className="p-4 text-sm text-slate-700 leading-relaxed">
            {details.internalFollowup.length === 0 ? (
              <div className="text-slate-500 text-center">لا توجد أي متابعات بعد.</div>
            ) : (
              <div className="space-y-4">
                {details.internalFollowup.map((item, idx) => {
                  if (item.type === "state") {
                    return (
                      <div key={`state-${idx}`} className="border border-slate-300 bg-slate-50 rounded p-3">
                        <div className="text-slate-500 text-xs mb-2 text-left md:text-right">
                          {formatDateTime(item.at)}
                        </div>
                        <div className="mb-1">
                          <span className="text-slate-500">حالة المعاملة الآن:</span>{" "}
                          <span className="font-semibold text-slate-800">{item.status}</span>
                        </div>
                        <div className="mb-1">
                          <span className="text-slate-500">ملاحظات:</span>{" "}
                          {item.notes ? <span className="text-slate-800">{item.notes}</span> : <span className="text-slate-400">لا توجد ملاحظات</span>}
                        </div>
                        <div className="text-xs text-slate-600 mt-2 leading-loose">
                          <div>
                            <span className="text-slate-500">الإدارة المتابعة:</span>{" "}
                            <span>{item.targetDepartment ? item.targetDepartment.name : "—"}</span>
                          </div>
                          <div>
                            <span className="text-slate-500">المكلَّف:</span>{" "}
                            <span>{item.assignedToUser ? item.assignedToUser.fullName : "—"}</span>
                          </div>
                        </div>
                      </div>
                    );
                    ...
                    return (
                      <div key={`log-${idx}`} className="border border-slate-300 rounded p-3">
                        <div className="text-slate-500 text-xs mb-2 text-left md:text-right">
                          {formatDateTime(item.at)}
                        </div>
                        <div className="mb-1">
                        ...
                        <div className="mb-1">
                          ...
                          {item.note ? <span className="text-slate-800">{item.note}</span> : <span className="text-slate-400">—</span>}
                        </div>
                        <div className="text-xs text-slate-600 mt-2 leading-loose">
                          <span className="text-slate-500">تم التحديث بواسطة:</span>{" "}
                          <span>{item.updatedBy ? item.updatedBy.fullName : "—"}</span>
                        </div>
                      </div>
                    );
                  }
        ...
        <div className="w-full bg-white rounded-lg shadow border border-slate-300 mb-16 text-slate-800">
          <div className="border-b border-slate-300 px-4 py-2 bg-slate-50 rounded-t text-center">
            <h3 className="text-lg font-semibold">إضافة متابعة / تحديث حالة</h3>
          </div>

          <div className="p-4 text-sm text-slate-700 leading-relaxed">
            {followupError && <div className="text-red-600 text-sm mb-3 text-center">{followupError}</div>}

            <form className="grid md:grid-cols-2 gap-4" onSubmit={handleAddFollowup}>
              <div className="flex flex-col">
                <label className="text-slate-600 mb-1">الحالة</label>
                <select className="border border-slate-300 rounded px-2 py-1 text-sm" value={newStatus} onChange={(e) => setNewStatus(e.target.value)}>
              ...
              {isPrivileged && (
                <div className="flex flex-col">
                  <label className="text-slate-600 mb-1">إحالة إلى إدارة</label>
                  <select
                  ...
                  <div className="text-[11px] text-slate-400 mt-1 leading-relaxed">في حال تحويل المعاملة لإدارة أخرى رسمياً.</div>
                </div>
              )}

              {isPrivileged && (
                <div className="flex flex-col md:col-span-2">
                  <label className="text-slate-600 mb-1">المكلَّف بمتابعة الموضوع</label>
                  <select
                    className="border border-slate-300 rounded px-2 py-1 text-sm"
                  ...
                  >
                    ...
                    {usersList.map((u) => (
                      <option key={u.id} value={u.id}>
                        {u.fullName}
                        {u.departmentId ? ` (إدارة ${u.departmentId})` : ""}
                      </option>
                    ))}
                  </select>
                  <div className="text-[11px] text-slate-400 mt-1 leading-relaxed">يُسجل كمسؤول متابعة رسمي لهذه المعاملة.</div>
                </div>
              )}
              ...
              <div className="md:col-span-2 flex justify-center md:justify-end">
                <button type="submit" disabled={submittingFollowup} className="bg-blue-600 text-white px-4 py-2 rounded disabled:opacity-50">
            ...
            <p className="text-[11px] text-slate-400 mt-4 leading-relaxed text-center md:text-right">
              ...
              <span className="font-semibold text-slate-600">SystemAdmin / DepartmentManager</span> يمكنهم تغيير الإدارة أو تكليف موظف. كل تغيير يُسجَّل بالاسم والتوقيت.
            </p>
          </div>
        </div>
      </div>
